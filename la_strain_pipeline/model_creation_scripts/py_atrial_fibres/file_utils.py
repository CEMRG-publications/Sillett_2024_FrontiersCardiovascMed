...
def reindex_elem(vtx_elem,
				 elem):
	
	elem_reindexed = copy.deepcopy(elem)
	for i,t in enumerate(elem):
		elem_reindexed[i,0] = np.where(vtx_elem==t[0])[0][0]
		elem_reindexed[i,1] = np.where(vtx_elem==t[1])[0][0]
		elem_reindexed[i,2] = np.where(vtx_elem==t[2])[0][0]
		elem_reindexed[i,3] = np.where(vtx_elem==t[3])[0][0]

	return elem_reindexed
