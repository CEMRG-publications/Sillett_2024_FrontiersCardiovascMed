...
def surf_find_LSPV_RSPV_posterior_points(la_pts,
    ...
    for v in lipv_vtx:
        ...
        if dot_prod_tmp<dot_prod:
            ...
            dot_prod = dot_prod_tmp
            print('The mesh is weird - please select LM points manually') 
...
def extract_boundaries(foldername,meshname,
                       clipper_name,
                       input_tags):

    la_pts,la_faces,la_tags = read_surf_polydata(foldername+meshname)
    ...
    np.savetxt(foldername+"/prodRefinedLandmarks.txt",landmarks,delimiter=',')
