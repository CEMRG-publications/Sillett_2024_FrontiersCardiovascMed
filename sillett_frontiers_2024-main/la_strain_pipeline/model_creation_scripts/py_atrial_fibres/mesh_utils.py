...
def map_tags_3d(phi_el_file,
	...
	tags_tt = np.zeros((phi.shape[0],),dtype=float)+default_tag
	for i in range(phi.shape[0]):
...
def connect_LA_RA_BB(meshname,
	...
	print("Finding points on the LA and RA Bachmann Bundle...")

	pts = read_pts(meshname+".pts")

	lon = read_lon(tagged_meshname+".lon")

	elem = read_elem(meshname+".elem",el_type="Tt",tags=True)
	tags_original = elem[:,-1]

	elem_in = read_elem(tagged_meshname+".elem",el_type="Tt",tags=True)
	tags = elem_in[:,-1]
	elem = elem_in[:,:4]

	elem_BB = np.where(tags==tag_BB)[0]

	eidx_LA = np.where(tags_original==tag_LA)[0]
	eidx_RA = np.where(tags_original==tag_RA)[0]

	eidx_LA_BB = eidx_LA[np.where(tags[eidx_LA]==tag_BB)[0]]
	eidx_LA_BB = np.array(eidx_LA_BB)

	eidx_RA_BB = eidx_RA[np.where(tags[eidx_RA]==tag_BB)[0]]
	eidx_RA_BB = np.array(eidx_RA_BB)

	elem_LA_BB = elem[eidx_LA_BB,:]
	vtx_LA_BB = np.unique(elem_LA_BB.flatten())
	pts_LA_BB = pts[vtx_LA_BB,:]

	elem_RA_BB = elem[eidx_RA_BB,:]
	vtx_RA_BB = np.unique(elem_RA_BB.flatten())
	pts_RA_BB = pts[vtx_RA_BB,:]	

	print("Finding LA-RA connections...")

	lines=[]	
	taken_la_bb = []	

	for i,point in enumerate(pts_RA_BB):
		distance = np.linalg.norm(pts_LA_BB-pts_RA_BB[i,:],axis=1)
		sorted_idx = np.argsort(distance)	

		id_min = sorted_idx[0]
		count = 0
		while id_min in taken_la_bb:
			id_min = sorted_idx[count+1]
			count += 1	

		taken_la_bb.append(id_min)

		lines.append([vtx_RA_BB[i], vtx_LA_BB[id_min], connection_tag])
		fiber = np.array([1,0,0,0,1,0], dtype=int)
		fiber2 = np.reshape(fiber,[1,6])
		lon = np.concatenate ((lon, [fiber]), axis=0)

	lines=np.array(lines)

	write_tets_ln(output_meshname+".elem",elem_in,lines)
	write_lon(lon,output_meshname+".lon")
